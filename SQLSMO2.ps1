...
#		$ENABLED
		...
		{	
			...
			{		
				...
				$null = Start-Job -ScriptBlock $RestoreDatabase -ArgumentList ($DESTSERVER,$BACKUPFILE, $DESTDB, $LOGFOLDER, $DATAFOLDER, $restore_options) 	#| Out-Null				
			}
...
#			$null = Start-Job -ScriptBlock $RestoreLogs -ArgumentList ($DESTSERVER,$BACKUPFILE, $DESTDB, $restore_options) 	#| Out-Null							
		...
		{ 
			$null = $running | Wait-Job #| Out-Null
		}		
	}
	$cnt++
}
$null = Get-Job | Receive-Job #| Out-Null
