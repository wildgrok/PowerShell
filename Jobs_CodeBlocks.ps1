...
function GetBackupInfo ($server, $backupfile)
{
  <#
      Sample use:
      $bkfile = '\\SERVERNAME\BACKUPS\DBName_backup_2015_05_11_000029_6076771.bak'
      $d = GetBackupInfo $server $bkfile
      foreach ($x in $d)
      {
      foreach ($y in $x.sqlfilelist)	{$y}
  ...
  $dict_backup = @{}
  $sqlfilelist 	= 'SET NOCOUNT ON RESTORE FILELISTONLY FROM DISK = ' + [char]39 + $backupfile + [char]39
  $dict_backup['sqlfilelist'] = Invoke-Sqlcmd3 ($server, $sqlfilelist, "master")
  return $dict_backup
}


function GetLatestBackup  ($url, $filter='*.bak')
<#	
    Returns folder contents sorted by modified date
    Sample use:
    $backupfolder = '\\SERVERNAME\Backups'
...
function RestoreDatabase ($server,$backupfile, $dbname1, $dbname2, $logfolder, $datafolder, $restore_options)
{
  #$dbname1 is source db, $dbname2 is destination db
  
  #This is the magic part
  #It is needed to import Invoke-Sqlcmd3, GetLatestBackup, GetBackupInfo used by RestoreDatabase and $RestoreLogs
  . C:\Users\jorgebe\Documents\OneDrive\Briefcase_CCL\PASS2019\Jobs_CodeBlocks.ps1	
  ...
  else
  {
    #Latest backup from folder
    $filter = $dbname1 + '_*.bak'
    $bkfile =  GetLatestBackup ($backupfile, $filter)
    $backupfile2 = $backupfile + '\' + $bkfile	
  }
	
  $dict_backup = GetBackupInfo ($server, $backupfile2)
  ...
  foreach( $x in $dict_backup.sqlfilelist)
  {
    ...
    {
            $suffix = 'Log'
            #$logfilename = $x.logicalname not using original logicalname
      $file_renamed = $dbname2 + '_' + $suffix + $ext
      $s = $s + ' MOVE ' + [char]39 + $x.logicalname + [char]39 + ' TO ' + [char]39 + $logfolder + '\' + $file_renamed + [char]39 + ', '
    }
        else  # any other type (D,F,S) we put in the Data folder
  ...
  $s1 = $s1 + ' FROM master..sysprocesses WHERE dbid = db_id(' + [char]39 + $dbname2 + [char]39 + ') '
  $s1 = $s1 + ' EXEC(@kill); '
  $null = Invoke-Sqlcmd3 ($server, $s1, "master")
  $null = Invoke-Sqlcmd3 ($server, $s, "master")
  if ($restore_options['recovery'] -eq 'RECOVERY')
  {
    $s1 = 'ALTER AUTHORIZATION ON DATABASE::[' + $dbname2 + '] TO [sa]' +  [char]13 + [char]10
    $null = Invoke-Sqlcmd3 ($server, $s1, $dbname2)
  }	
...
function RestoreLogs ($server,$backupfile, $dbname1, $dbname2, $restore_options)
{
  # Remember the magic: 
  . C:\Users\jorgebe\Documents\OneDrive\Briefcase_CCL\PASS2019\Jobs_CodeBlocks.ps1	
  <#
